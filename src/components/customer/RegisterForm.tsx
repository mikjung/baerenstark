 ...
 *
 * Iteration 5 (US-31):
 *   - OAuth-Buttons (Google + GitHub) UNTER dem Pw-Formular,
 *     wenn `NEXT_PUBLIC_FEATURE_OAUTH_LOGIN === 'true'`.
 *   - Fehler-Mapping für `?error=oauth_no_email` etc.
 *
...
export function RegisterForm() {
  ...
  const errorQuery = searchParams?.get('error') ?? null;
  ...
  return (
    <div className="space-y-4">
      ...
      {oauthEnabled && (
        <>
          <OAuthDivider label="oder direkt anmelden mit" />
          <OAuthButtons heading="Oder direkt anmelden mit:" />
        </>
      )}
    </div>
  );
